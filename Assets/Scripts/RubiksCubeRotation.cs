    ...
    void Update()
{
    ...
    // Face rotations - adjusted for 180° Y rotation
    ...
    if (Input.GetKeyDown(KeyCode.R))
    {
        ...
        StartCoroutine(RotateFace(Vector3.left, Vector3.right, 90f));
    }
    if (Input.GetKeyDown(KeyCode.L))
    {
        Debug.Log("L key pressed - rotating Left face clockwise");
        StartCoroutine(RotateFace(Vector3.right, Vector3.left, 90f));
    }
    ...
    if (Input.GetKeyDown(KeyCode.D))
    {
        ...
        StartCoroutine(RotateFace(Vector3.down, Vector3.down, 90f));
    }
